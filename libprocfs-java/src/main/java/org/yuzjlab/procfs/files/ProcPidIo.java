...
public class ProcPidIo {
   ...
   * sum of bytes which this process passed to read(2) and similar system calls. It includes things
   * such as terminal I/O and is unaffected by whether or not actual physical disk I/O was required
   ...
   * read syscalls
   *
   * <p>Attempt to count the number of read I/O operations that is, system calls such as read(2) and
   * pread(2).
   */
   ...
   * bytes read
   *
   * <p>Attempt to count the number of bytes which this process really did cause to be fetched from
   ...
   * task truncates some dirty pagecache, some I/O which another task has been accounted for (in its
