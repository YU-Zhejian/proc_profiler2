...
/**
 * NOTE: CPU time , measured in units of USER_HZ (1/100ths of a second on most architectures, use
 * sysconf(_SC_CLK_TCK) to obtain the right value), that the system ("cpu" line) or the specific CPU
...
public class ProcStat {
   ...
   *
   * <p>The CPU will not wait for I/O to complete; iowait is the time that a task is waiting for I/O
   * to complete. When CPU goes into idle state for outstanding task I/O, another task will be
   * scheduled on this CPU.
   *
   * <p>On a multi-core CPU, the task waiting for I/O to complete is not running on any CPU, so the
   * iowait of each CPU is difficult to calculate.
   *
   * <p>The value in this field may decrease in certain conditions.
   */
  ...
  /**
   * Stolen time, which is the time spent in other operating systems when running in a virtualized
   * environment
   */
  ...
  /**
   * Time spent running a niced guest (virtual CPU for guest operating systems under the control of
   * the Linux kernel).
   */
  ...
  protected static long getTrailingLong(List<String> lines, String prefix)
    ...
    if (reqLine == null) {
      throw new ProcessBaseException(
          new RuntimeException("proc/stat file contains no %s line!".formatted(prefix)));
    }
    ...
    try {
      ...
      throw new ProcessBaseException(
          new RuntimeException(
              "proc/stat file contains illegal %s line ('%s')!".formatted(prefix, reqNum)));
    }
  }

  public float getCpuGuestSeconds() {
    return (float) this.cpuGuest / SystemInfo.CLOCK_TICK;
  }

  public float getCpuUserSeconds() {
    return (float) this.cpuUser / SystemInfo.CLOCK_TICK;
  }

  public float getCpuStealSeconds() {
    return (float) this.cpuSteal / SystemInfo.CLOCK_TICK;
  }

  public float getCpuIrqSeconds() {
    return (float) this.cpuIrq / SystemInfo.CLOCK_TICK;
  }

  public float getCpuSoftIrqSeconds() {
    return (float) this.cpuSoftIrq / SystemInfo.CLOCK_TICK;
  }

  public float getCpuIOWaitSeconds() {
    return (float) this.cpuIOWait / SystemInfo.CLOCK_TICK;
  }

  public float getCpuIdleSeconds() {
    return (float) this.cpuIdle / SystemInfo.CLOCK_TICK;
  }

  public float getCpuSystemSeconds() {
    return (float) this.cpuSystem / SystemInfo.CLOCK_TICK;
  }

  public float getCpuNiceSeconds() {
    return (float) this.cpuNice / SystemInfo.CLOCK_TICK;
  }

  public float getCpuGuestNiceSeconds() {
    return (float) this.cpuGuestNice / SystemInfo.CLOCK_TICK;
  }
