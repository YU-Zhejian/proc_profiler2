...
public final class ProcPidStat {
     ...
     * (<i>policy</i> below; see <code>sched_setscheduler(2)</code>), this is the negated scheduling
     * priority, minus one; that is, a number in the range -2 to -100, corresponding to real-time
     * priorities 1 to 99. For processes running under a non-real-time scheduling policy, this is
     * the raw nice value ( <code>setpriority(2)</code>) as represented in the kernel. The kernel
    ...
    /**
     * The nice value (see <code>setpriority(2)</code>), a value in the range 19 (low priority) to
     * -20 (high priority).
     */
    ...
    /**
     * Scheduling policy (see <code>sched_setscheduler(2)</code>). Decode using the SCHED_*
     * constants in <i>linux/sched.h</i>.
     */
