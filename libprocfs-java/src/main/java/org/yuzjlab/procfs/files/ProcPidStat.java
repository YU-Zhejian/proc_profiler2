 ...
 * pages</a>.
 *
 * <p><i>/proc/[pid]/stat</i> Status information about the process. This is used by <code>ps(1)
 * </code>. It is defined in the kernel source file <i>fs/proc/array.c</i>.
 *
 * <p>The fields, in order, with their proper <code>scanf(3)</code> format specifiers, are listed
 * below. Whether or not certain of these fields display valid information is governed by a ptrace
 * access mode <b>PTRACE_MODE_READ_FSCREDS </b>|<b>PTRACE_MODE_NOAUDIT</b>chec (refer to <code>
 * ptrace(2)</code>). If the check denies access, then the field value is displayed as 0. The
...
public final class ProcPidStat {
  ...
  /**
   * The filename of the executable, in parentheses. Strings longer than <b>TASK_COMM_LEN </b>(16)
   ...
   * <p>R Running
   *
   * <p>S Sleeping in an interruptible wait
   *
   * <p>D Waiting in uninterruptible disk sleep
   *
   * <p>Z Zombie
   *
   * <p>T Stopped (on a signal) or (before Linux 2.6.33) trace stopped
   *
   * <p>t Tracing stop (Linux 2.6.33 onward)
   *
   * <p>W Paging (only before Linux 2.6.0)
   *
   * <p>X Dead (from Linux 2.6.0 onward)
   *
   * <p>x Dead (Linux 2.6.33 to 3.13 only)
   *
   * <p>K Wakekill (Linux 2.6.33 to 3.13 only)
   *
   * <p>W Waking (Linux 2.6.33 to 3.13 only)
   *
   ...
   * kernel source file <i>include/linux/sched.h</i>. Details depend on the kernel version.
   */
   ...
   * (divide by <i>sysconf(_SC_CLK_TCK)</i>). This includes guest time, <i>guest_time</i> (time
   ...
   * measured in clock ticks (divide by <i>sysconf(_SC_CLK_TCK)</i>). (See also <code>times(2)
   * </code>.) This includes guest time, <i>cguest_time</i> (time spent running a virtual CPU, see
   * below).
   */
  ...
  /**
   * (Explanation for Linux 2.6) For processes running a real-time scheduling policy (<i>policy</i>
   * below; see <code>sched_setscheduler(2)</code>), this is the negated scheduling priority, minus
   * one; that is, a number in the range -2 to -100, corresponding to real-time priorities 1 to 99.
   * For processes running under a non-real-time scheduling policy, this is the raw nice value (
   * <code>setpriority(2)</code>) as represented in the kernel. The kernel stores nice values as
   * numbers in the range 0 (high) to 39 (low), corresponding to the user-visible nice range of -20
   * to 19.
   *
  ...
  /**
   * The nice value (see <code>setpriority(2)</code>), a value in the range 19 (low priority) to -20
   * (high priority).
   */
  ...
  /**
   * The time in jiffies before the next <b>SIGALRM </b>is sent to the process due to an interval
   ...
   * count toward text, data, or stack space. This does not include pages which have not been
   * demand-loaded in, or which are swapped out. This value is inaccurate; see
   * <i>/proc/[pid]/statm</i> below.
   */
  public final BigInteger rss;

  /**
   * Current soft limit in bytes on the rss of the process; see the description of <b>RLIMIT_RSS
   * </b>in <code>getrlimit(2)</code>.
   */
   ...
   * provide information on real-time signals; use <i>/proc/[pid]/status</i> instead.
   */
   ...
   * real-time policy, or 0, for non-real-time processes (see <code>sched_setscheduler(2)</code>).
   */
  public final int rtPriority;

  /**
   * Scheduling policy (see <code>sched_setscheduler(2)</code>). Decode using the SCHED_* constants
   * in <i>linux/sched.h</i>.
   */
